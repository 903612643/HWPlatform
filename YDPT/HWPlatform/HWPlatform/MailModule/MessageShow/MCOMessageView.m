...
@interface MCOMessageView () <MCOHTMLRendererIMAPDelegate, UIGestureRecognizerDelegate, MCOHTMLRendererDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if(self) {
        ...
        NSLog(@"with = %f", [self bounds].size.width);
        NSLog(@"heigh = %f", [self bounds].size.height);
...
- (void) _refresh
{
    ...
    if (_message == nil) {
        ...
        if ([_message isKindOfClass:[MCOMessageBuilder class]]) {
...
- (void)handleTapAtpoint:(CGPoint)pt
{
    if (_gestureRecognizerEnabled)
    {
        ...
        NSString *tags = [_webView stringByEvaluatingJavaScriptFromString:
                          [NSString stringWithFormat:@"getHTMLElementsAtPoint(%f,%f);",(CGFloat)pt.x,(CGFloat)pt.y]];
                
        NSString *tagsSRC = [_webView stringByEvaluatingJavaScriptFromString:
                             [NSString stringWithFormat:@"getLinkSRCAtPoint(%f,%f);",(CGFloat)pt.x,(CGFloat)pt.y]];
        
        NSString *tagsID = [_webView stringByEvaluatingJavaScriptFromString:
                             [NSString stringWithFormat:@"getObjectIdAtPoint(%f,%f);",(CGFloat)pt.x,(CGFloat)pt.y]];
