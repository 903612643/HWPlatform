...
protocol BubbleControlDelegate:NSObjectProtocol
{
    //取消全屏委托
    func setHiddenBar()
}
...
class BubbleControl: UIControl {
    ...
    //取消全屏委托
    var delegateSetBar:BubbleControlDelegate?
    ...
    var CurrentPoint:CGPoint =  CGPointMake(0, 0);
    ...
    var toggle: Bool = false {
        didSet {
            didToggle? (toggle)
            if toggle {
                openContentView() // colin modify it
            } else {
                closeContentView() // colin modify it
            }
        }
    }
    ...
    func defaultInit () {
        ...
        // events
        addTarget(self, action: "touchDown:event:", forControlEvents: UIControlEvents.TouchDown)
        addTarget(self, action: "touchUp:event:", forControlEvents: UIControlEvents.TouchUpInside)
        ...
        NSNotificationCenter.defaultCenter().addObserver(self,
            selector: "rotate_bubble:",
            name: UIDeviceOrientationDidChangeNotification,
            object: nil)
    }
    
    /// @brief 设置方向变化通知处理
    func rotate_bubble(sender: NSNotification?) {
    ...
    func snap () {
        ...
        if center.y > window.h {
            let targety = window.h - w
            //var badgeTargetY = 0
            moveY(targety)
        }
    }
    ...
    func touchDown (sender: BubbleControl, event: UIEvent) {
        //bubble()
        let SenderX = sender.frame.origin.x
        let SenderY = sender.frame.origin.y
        
        CurrentPoint = CGPointMake(SenderX, SenderY);
//        print("TouchDown  SenderX == %f, SenderY == %f", SenderX, SenderY)
//        print("TouchDown   CurrentPointX == %f, CurrentPointY == %f", CurrentPoint.x, CurrentPoint.y)
    }
    
    func touchUp (sender: BubbleControl, event: UIEvent) {
        
        if bubbleState == .Snap {
//            print("点击事件！！！！")
//            var PostDIC = Dictionary<String, String>()
//            PostDIC["TouchMode"] = "点击事件"
//            
//            NSNotificationCenter.defaultCenter().postNotificationName("showDebugNotification",
//                object: nil, userInfo: PostDIC)
            toggle = !toggle
        } else {
            
            let XRange = abs(CurrentPoint.x - sender.frame.origin.x)
            let YRange = abs(CurrentPoint.y - sender.frame.origin.y)
            
            if XRange < 20.0 && YRange < 20.0
            {
                toggle = !toggle
                
                CurrentPoint = CGPointMake(CurrentPoint.x + sender.frame.size.width/2, CurrentPoint.y + sender.frame.size.height/2);
                center = CurrentPoint
                lockInWindowBounds()
            }
            else
...
//            }
            v.hidden = false
            self.hidden = true
            if((delegateSetBar) != nil){
                self.delegateSetBar?.setHiddenBar()
            }
        }
    }
