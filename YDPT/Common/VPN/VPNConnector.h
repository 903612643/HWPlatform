...
//    id<VPNConnectorDelegate> delegate;
    NSString *vpnIp;        //vpn设备IP地址
    short port;             //vpn设备端口号，一般为443
    NSString *userName;     //用户名认证的用户名
    NSString *password;     //用户名认证的密码
    NSString *authTimeout;  //认证时候连接vpn的超时时间
}
@property(weak,nonatomic)id<VPNConnectorDelegate> delegate;
